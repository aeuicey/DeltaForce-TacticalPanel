...
- **本分支 → 上游**：新功能与修复先在本分支开发验证，再通过 Pull Request 回馈上游。已被上游采纳的改进包括：Android 导出修复（PR #1）、绘图组件锁定（PR #3，上游在其基础上重构进了正式版）、套索锁定保护（PR #4）、兵棋路线鼠标跟随预览（PR #5）。
- **上游 → 本分支**：上游正式版的内容会定期合并回本分支（当前已合并至 v0.1.2）。

### 版本号关系

| 版本线 | 含义 | 当前版本 |
| --- | --- | --- |
| **上游正式版** | 上游仓库发布的稳定版本，功能经过验证 | `v0.1.2` |
| **本分支测试版** | 本仓库的开发测试版本号，含义为「**已合并上游 v0.1.2 全部内容** + 本分支独占功能（网页分享模式、Android 局域网协作、开屏视频等），尚未回馈上游的部分」 | `0.0.2-indoor` |

即：**`0.0.2-indoor` = 上游 `v0.1.2` + fork 独占功能**。本分支版本号独立于上游编号演进，不表示落后于上游；每次合并上游正式版后会在此注明对应关系。本分支的构建产物（Pages Demo、Windows 安装包、Android APK、Docker 镜像）均为测试版通道，稳定正式版请以上游仓库为准。
...
- 更新烬区 PC/移动端、攻防/胜者为王双端双模式的据点与活动区范围；其中胜者为王移动端 S2 守方复活点可部署载具改为步战车，A 点刷新载具改为主战坦克。
- 完成堑壕战 PC/移动端胜者为王模式的据点与活动区范围适配。
- 胜者为王模式的 PC 端与移动端数据正式分离，两端配置可独立维护与固化。
- 优化移动端 UI 展示，支持自定义地图图层元素大小。
- 据点与复活点的文字备注可在左侧边栏“地图图层”功能区关闭。
- 兵棋路线绘制新增鼠标跟随预览，感谢社区协作者 [@aeuicey](https://github.com/aeuicey) 提交 [PR #5](https://github.com/Deng0430/delta-force/pull/5)。
- 修复移动端黑屏问题。
