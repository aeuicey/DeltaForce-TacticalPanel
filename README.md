...
- **上游 → 本分支**：上游正式版的内容会定期合并回本分支（当前已合并至 v0.1.1）。

### 版本号关系

| 版本线 | 含义 | 当前版本 |
| --- | --- | --- |
| **上游正式版** | 上游仓库发布的稳定版本，功能经过验证 | `v0.1.1` |
| **本分支测试版** | 本仓库的开发测试版本号，含义为「**已合并上游 v0.1.1 全部内容** + 本分支独占功能（网页分享模式、Android 局域网协作、开屏视频等），尚未回馈上游的部分」 | `0.0.2-indoor` |

即：**`0.0.2-indoor` = 上游 `v0.1.1` + fork 独占功能**。本分支版本号独立于上游编号演进，不表示落后于上游；每次合并上游正式版后会在此注明对应关系。本分支的构建产物（Pages Demo、Windows 安装包、Android APK、Docker 镜像）均为测试版通道，稳定正式版请以上游仓库为准。
